...
export function mailOrderShipped({ to, orderRef, trackingNumber, carrier, storeName }) {
  const trackingLine = trackingNumber
    ? `<p style="margin:10px 0;color:#374151">Tracking number: <strong>${trackingNumber}</strong>${carrier ? ` (${carrier})` : ''}</p>`
    : '';
  ...
  return sendMail({
    ...
    html: `<div style="font-family:sans-serif;font-size:13px;max-width:560px;margin:0 auto;color:#111827">
      ${logoHeader}
      <p style="font-size:15px;font-weight:700;color:#0b6b6a;margin:0 0 8px">Your order is on its way!</p>
      <p style="margin:0 0 10px;color:#374151">Great news — <strong>${storeName || 'your seller'}</strong> has shipped your order <strong>${orderRef}</strong>.</p>
      ${trackingLine}
...
export function mailNewMessage({ to, senderName, productName, role, convoId }) {
  ...
  return sendMail({
    ...
    html: `<div style="font-family:sans-serif;font-size:13px;max-width:560px;margin:0 auto;color:#111827">
      ${logoHeader}
      <p style="font-size:15px;font-weight:700;color:#0b6b6a;margin:0 0 8px">You have a new message</p>
      <p style="margin:0 0 10px;color:#374151"><strong>${senderName}</strong> sent you a message about <strong>${productName}</strong>.</p>
      <p style="margin:16px 0"><a href="${link}" style="background:#0b6b6a;color:#fff;padding:8px 16px;border-radius:6px;text-decoration:none;font-size:13px;font-weight:600">View Message</a></p>
      <hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0">
      <p style="font-size:11px;color:#9ca3af">Sell4Life · Reply directly in your account.</p>
    </div>`,
  });
}
