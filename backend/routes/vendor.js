...
router.post('/create', authMiddleware, async (req, res) => {
  try {
    ...
    // Email verification is now enforced globally in authMiddleware —
    // no per-route check needed here.
...
// CJ product sync (images/videos/variant cjVid) now lives in a shared util —
// also used by the auto-sync-on-save hook in products.js and the periodic
// sweep worker, so all entry points behave identically.
...
router.post('/products/bulk-fetch-cj-images', authMiddleware, requireApprovedVendor, requireTier('professional'), express.json(), async (req, res) => {
  ...
  try {
    ...
    let fallbackCount = 0, sampleCjSearchError = null, sampleCjSearchDebug = null;
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      if (r.status === 'updated') {
        ...
        if (r.note === 'variant-fallback') {
          ...
          if (!sampleCjSearchDebug && r.cjSearchDebug) sampleCjSearchDebug = r.cjSearchDebug;
        }
    ...
    send({
      ...
      debug: { noCjVariants, noCategoryReturned, sampleVideoApiDebug, fallbackCount, sampleCjSearchError, sampleCjSearchDebug },
    });
  } catch (err) {
