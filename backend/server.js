...
// ======================================================
// TEMP DEBUG — READ-ONLY check of CJ warehouse/inventory data ahead of
// deploying the shipping-origin fix. Fetches CJ's raw variant/inventory
// data and, if any non-CN stock is found, a live freight quote from that
// origin — never writes to the database. Remove after use.
// ======================================================
app.get('/api/_debug_cj_origin_test', async (req, res) => {
  if (req.query.k !== 's4l-debug-20260911b') return res.status(404).end();
  try {
    ...
    // cherry-picked here) — reimplemented inline, read-only, just for
    // this check.
    function candidateOrigins(inventories) {
