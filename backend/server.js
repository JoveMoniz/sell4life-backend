...
// ======================================================
// TEMP DEBUG — key-gated. Runs the real, existing "Sync from CJ" action
// (utils/cjProductSync.js syncProductFromCj) on one specific product —
// the exact same code path the vendor's own Sync button calls — and
// reports the before/after shippingCost/shippingOriginCountry. Used to
// confirm the fixed origin-preserving sync actually pulls CJ's real
// live freight quote through into the stored price. Remove after use.
// ======================================================
app.get('/api/_debug_run_sync', async (req, res) => {
  if (req.query.k !== 's4l-debug-20260911k') return res.status(404).end();
  try {
    const Product = (await import('./models/product.js')).default;
    const Vendor = (await import('./models/vendor.js')).default;
    ...
    res.json({
      name: before.name,
      syncResult,
      before: { shippingCost: before.shippingCost, shippingOriginCountry: before.shippingOriginCountry },
      after: { shippingCost: after.shippingCost, shippingOriginCountry: after.shippingOriginCountry },
    });
  } catch (err) {
