...
app.get('/api/_debug_draft_audit', async (req, res) => {
  if (req.query.k !== 'draft-audit-6b13') return res.status(404).end();
  try {
    ...
    const drafts = await Product.find({ vendor: vendor._id, active: false })
      .select('name description shortDescription bulletPoints images price stock trackInventory shippingCost shippingScope category subcategory variants comparePrice');
