...
// ======================================================
// TEMP DEBUG — read-only, key-gated. Checks whether the vendor's bulk
// "Sync from CJ" run has actually landed new prices yet, for a set of
// known-matched sample products plus a fresh overall coverage count.
// Remove after use.
// ======================================================
app.get('/api/_debug_sync_progress', async (req, res) => {
  ...
  try {
    ...
    const sampleIds = ['6aa2c9f7eae8170a20915088', '6aa2c9f7eae8170a2091508c', '6aa2c9f7eae8170a20915084'];
    const samples = await Product.find({ _id: { $in: sampleIds } })
      .select('name shippingCost shippingOriginCountry updatedAt').lean();
    ...
    for (const vendor of vendors) {
      ...
      const recentlyUpdated = cjProducts.filter(p => p.updatedAt && (Date.now() - new Date(p.updatedAt).getTime()) < 2 * 60 * 60 * 1000).length;
      coverage.push({
        ...
        updatedInLast2h: recentlyUpdated,
      });
    }
    res.json({ samples, coverage });
  } catch (err) {
